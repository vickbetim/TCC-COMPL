...
if (isset($_POST['btnEnviar'])) {
    ...
    $sql = "SELECT * FROM tb_clientes WHERE gmail = :gmail AND senha = :senha";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if ($tipocliente === 'C') {
            ...
            header('Location: pagina.php'); // Altere para a página correta
            
           
exit();
        } elseif ($tipocliente === 'F') {
            // Se for um funcionário (F), redirecione para a página de funcionário
            header('Location: homeadm.php'); // Altere para a página correta
            exit();
        }
    } else {
        // Login inválido (nenhum registro correspondente encontrado)
    ...
    <div class="container">
      <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        ...
        <div class="button-container">
          ...
          <p>
            Ainda não tem uma conta? <a href="cliente.php">Cadastre-se</a>
          </p>
        </div>
      </form>
    </div>
